...
#define TF_LITE_MICRO_TESTS_END                                       \
  printf("%d/%d tests passed", micro_test::tests_passed,         \
...
#define TF_LITE_MICRO_EXPECT(x)                               \
  do {                                                        \
    if (!(x)) {                                               \
      printf(#x " failed at %s:%d", __FILE__, __LINE__); \
...
#define TF_LITE_MICRO_EXPECT_EQ(x, y)                                    \
  do {                                                                   \
    ...
    auto vy = y;                                                         \
    if ((vx) != (vy)) {                                                  \
      printf(#x " == " #y " failed at %s:%d (%d vs %d)", __FILE__,  \
...
#define TF_LITE_MICRO_EXPECT_TRUE(x)                                       \
  do {                                                                     \
    if (!(x)) {                                                            \
      printf(#x " was not true failed at %s:%d", __FILE__, __LINE__); \
      micro_test::did_test_fail = true;                                    \
    }                                                                      \
  } while (false)
...
#define TF_LITE_MICRO_EXPECT_FALSE(x)                                       \
  do {                                                                      \
    if (x) {                                                                \
      printf(#x " was not false failed at %s:%d", __FILE__, __LINE__); \
      micro_test::did_test_fail = true;                                     \
    }                                                                       \
  } while (false)
...
#define TF_LITE_MICRO_FAIL(msg)                       \
  do {                                                \
    printf("FAIL: %s", msg, __FILE__, __LINE__); \
    micro_test::did_test_fail = true;                 \
  } while (false)
  ...
  do {                                                                       \
    for (int i = 0; string1[i] != '\0' && string2[i] != '\0'; i++) {         \
      if (string1[i] != string2[i]) {                                        \
        printf("FAIL: %s did not match %s", string1, string2, __FILE__, \
                    __LINE__);                                               \
        micro_test::did_test_fail = true;                                    \
      }                                                                      \
    }                                                                        \
